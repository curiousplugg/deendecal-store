#!/usr/bin/env python3
"""
Late API Social Media Post Scheduler - DeenDecal5
...
Time windows are shifted 1 hour forward from deendecal1.
"""
...
PROFILE_NAME = os.getenv("LATE_PROFILE_NAME", "deendecal5")
TIMEZONE = os.getenv("TIMEZONE", "America/New_York")
VIDEO_DIR = Path(__file__).parent / os.getenv("VIDEO_DIR", "used2")

# Posting time windows (hour ranges) - shifted 1 hour forward from deendecal1
POSTING_WINDOWS = [
    (6, 7),   # 6-7 AM (was 5-6 AM)
    (8, 9),   # 8-9 AM (was 7-8 AM)
    (11, 12), # 11 AM-12 PM (was 10-11 AM)
    (13, 14), # 1-2 PM (was 12-1 PM)
    (15, 16), # 3-4 PM (was 2-3 PM)
    (16, 17), # 4-5 PM (was 3-4 PM)
    (18, 19), # 6-7 PM (was 5-6 PM)
    (20, 21), # 8-9 PM (was 7-8 PM)
    (22, 23), # 10-11 PM (was 9-10 PM)
    (0, 1),   # 12-1 AM (was 11-12 PM) - midnight to 1am
]

# Account IDs (hardcoded for deendecal5)
TIKTOK_ACCOUNT_ID = "6928a59a6206180ff197e9ec"
INSTAGRAM_ACCOUNT_ID = "6928a58df43160a0bc9994fb"
YOUTUBE_ACCOUNT_ID = "6928a5ba36fafcd7c418d424"
PROFILE_ID = "6928a5736206180ff197e815"
...
def get_random_time_in_window(start_hour: int, end_hour: int) -> Tuple[int, int]:
    ...
    # Handle midnight window (0-1) - end_hour is 1, so we want 0:00 to 0:59
    if start_hour == 0:
        hour = 0
...
def format_datetime_for_api(date: datetime, hour: int, minute: int, timezone: str) -> str:
    ...
    # Handle midnight window - if hour is 0, it's the same day (midnight to 1am)
...
def main():
    ...
    print("🚀 Starting Late API Post Scheduler - DeenDecal5")
    ...
    for date in dates:
        ...
        for i, window in enumerate(POSTING_WINDOWS):
            ...
            # Handle midnight window (0-1) - it's still the same day
            scheduled_date = date
            scheduled_time = format_datetime_for_api(scheduled_date, hour, minute, TIMEZONE)
